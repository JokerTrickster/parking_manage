...
func (r *HistoryParkingRepository) GetHistory(ctx context.Context, projectID string) ([]mysql.ExperimentSessions, error) {
	...
	result := r.GormDB.WithContext(ctx).Where("project_id = ?", projectID).Order("created_at DESC").Find(&experimentSessions)
	if result.Error != nil {
...
func (r *HistoryParkingRepository) FindCctvResultByExperimentSessionID(ctx context.Context, experimentSessionID int) ([]string, error) {
	...
	result := r.GormDB.WithContext(ctx).Where("experiment_session_id = ?", experimentSessionID).Order("created_at DESC").Find(&cctvResults)
	if result.Error != nil {
		return nil, result.Error
	}
