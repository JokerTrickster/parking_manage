...
type TestUploadParkingUseCase struct {
	Repository     _interface.ITestUploadParkingRepository
	ContextTimeout time.Duration
}

func NewTestUploadParkingUseCase(repo _interface.ITestUploadParkingRepository, timeout time.Duration) _interface.ITestUploadParkingUseCase {
	return &TestUploadParkingUseCase{Repository: repo, ContextTimeout: timeout}
}

func (d *TestUploadParkingUseCase) TestUpload(c context.Context, projectID string, files []*multipart.FileHeader) (response.ResTestUpload, error) {
	...
	if err := os.MkdirAll(targetPath, 0755); err != nil {
		return response.ResTestUpload{}, err
	}
