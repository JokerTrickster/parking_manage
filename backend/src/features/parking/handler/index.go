...
func NewParkingHandler(e *echo.Echo) error {
	...
	cctvImagesRepo := repository.NewCctvImageParkingRepository(mysql.GormMysqlDB)
	...
	deploymentResultRepo := repository.NewDeploymentResultRepository(mysql.GormMysqlDB)
	...
	cctvImagesUseCase := usecase.NewCctvImageParkingUseCase(cctvImagesRepo, 30*time.Second)
	...
	deploymentResultUseCase := usecase.NewDeploymentResultUseCase(deploymentResultRepo, 30*time.Second)
	...
	NewCctvImageParkingHandler(e, cctvImagesUseCase)
	...
	NewDeploymentResultHandler(e, deploymentResultUseCase)
